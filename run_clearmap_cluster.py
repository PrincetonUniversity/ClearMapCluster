...
from ClearMap.cluster.preprocessing import updateparams, arrayjob, pth_update, listdirfull, process_planes_completion_checker
...
if __name__ == '__main__':
    ...
    if stepid == 0:
        ...
        if not os.path.exists(os.path.join(params['outputdirectory'], 'clearmap_cluster')): shutil.copytree(os.getcwd(), os.path.join(params['outputdirectory'], 'clearmap_cluster'), ignore=shutil.ignore_patterns('^.git')) #copy run folder into output to save run inf
