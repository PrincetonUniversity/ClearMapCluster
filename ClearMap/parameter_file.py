...
def set_parameters_for_clearmap(testing=False, **kwargs):
    ...
    Editing of this function is how one changes cell detection parameters
    """
    ...
    ######################### Cell Detection Parameters using custom filter    #Spot detection method: faster, but optimised for spherical objects.
    ...
    #set directories
    if not testing:
        savedirectory = os.path.join(kwargs["outputdirectory"], "ClearMapClusterOutput")
    ...
    StackProcessingParameter = {
        ...
        "chunkOptimization" : True,        #increase chunk size for optimization (True, False or all = automatic)
        "chunkOptimizationSize" : True, #zmd changed to false to see if this was messing up planes issue, 202001019
