...
#%%
def sequentiallyProcessStack_usingCluster(jobid, source, x = all, y = all, z = all, sink = None,
                         processes = 2, chunkSizeMax = 30, chunkSizeMin = 10, chunkOverlap = 10,
                         chunkOptimization = True, chunkOptimizationSize = all,
                         function = noProcessing, join = joinPoints, verbose = False, **parameter):
    ...
    #tp added for cluster
    if jobid > nSubStacks:
        print ('JobID {} > nSubStacks {}'.format(jobid, nSubStacks))
